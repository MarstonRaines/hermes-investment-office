...
② Parquet 时间序列（经版本感知视图，ts04 §2.6）：ohlcva/v1 / financial_history/v1 /
                          etf_holdings/v2（v1 兼容读取） / index_history/v1
...
| 持仓 | `etf_holding_snapshots`（header）+ `etf_holdings/v2`（v1 兼容读取） | Level 1 穿透 | QDII/穿透必需 |
...
| **Level 1** | 最新披露持仓 | `etf_holding_snapshots` + `etf_holdings/v2`（v1 兼容读取） | **`disclosure_date`（披露日）** | 披露完整性（前十大 vs 全持仓） |
| **Level 2** | 估算 exposure（未来能力，当前切片未实现） | 预留给引擎派生（L1 + `index_history` 权重近似） | — | 当前必须 `status=NOT_IMPLEMENTED`、`is_estimate=false`；不得输出 `ESTIMATE` |
...
| Concentration | 前十大成分权重占比 / 行业集中度 | `etf_holdings/v2`（L1，v1 兼容读取）或指数权重 | 穿透口径携带 disclosure_date |
...
| ETF Overlap | 两只 ETF 持仓重叠度（`Jaccard = |A∩B| / |A∪B|`，按 L1 权重口径） | `etf_holdings/v2`（L1，v1 兼容读取） | `overlap_pairs`（pair + score + as_of_date） |
...
1. **穿透默认 Level 1**（最新披露持仓）：暴露计算使用 `etf_holdings/v2` 的 `weight_ratio`（逐行披露百分比 / 100，0–1；v1 仅兼容读取，ts04 §2.4）；`as_of` = `disclosure_date`；
2. **Level 2（估算 exposure）**：当前 M3 切片未实现；输出必须携带 `status=NOT_IMPLEMENTED`、`is_estimate=false`，不得伪装成 `ESTIMATE`。未来启用估算必须先补 ADR、配置和测试；
