...
├── etf_holdings/v1/      # 历史兼容读取
├── etf_holdings/v2/      # 当前写入（单行 weight_pct / 100）
└── index_history/v1/
```
...
| `etf_holdings` | `v2`（v1 兼容读取） | ETF 披露持仓明细（Level 1） | `etf_holding_snapshots`（header） | etf_holdings | etf 模块 |
...
### 2.4 `etf_holdings/v1` Schema（历史兼容读取）
...
### 2.4.1 `etf_holdings/v2` Schema（当前写入）

v2 与 v1 保持相同列集合、`holding_snapshot_id` 及 PG header 指针语义；差异仅在
`weight_ratio`：逐行执行 `weight_pct / 100`，不按当前披露行总和再次归一化。
v1 只用于历史兼容读取，读取器依据 PG `parquet_path` 中的版本目录选择解析契约；
缺少明确 `v1`/`v2` 版本的路径拒绝读取。v2 新写入不会覆盖 v1 文件。
...
| `etf_holdings/v1`、`etf_holdings/v2` | 永久 | 同上；按 PG header 的唯一 `parquet_path` 读取 |
