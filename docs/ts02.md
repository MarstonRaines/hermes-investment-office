...
| parquet_path | TEXT | NULL | `etf_holdings/v1/…` 全量明细 |
| provenance_id | UUID | FK → provenance_records | |
| created_at | TIMESTAMPTZ | NOT NULL | |
...
4. **Parquet schema v1 的具体列**：由 TS-04 Data Contract 冻结（`ohlcva/v1/`、`financial_history/v1/`、`etf_holdings/v1/`）。
