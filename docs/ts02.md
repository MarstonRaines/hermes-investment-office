...
| provenance_id | UUID | NULL，FK → provenance_records | 派生结果血缘；COMPLETED 时必填，`source_kind=DERIVED_ENGINE` |
...
> **不可变性（冻结）**：`COMPLETED` 后 assumption / input ref / engine_version / result / provenance 全部禁止修改（服务层拒绝 + 无 UPDATE 路径）；需要新假设 = 新 Run。`SUPERSEDED` 由"更新的已批准 run"触发，旧 run 保留。派生 provenance 与 run 同事务写入。
